...
import folk.sisby.antique_atlas.util.AtlasManager;
...
public class AntiqueAtlas implements ClientModInitializer {
    ...
    public static AtlasManager getAtlasIdData(World world) {
        if (world.isClient()) {
            LOGGER.warn("Tried to access server only data from client.");
            return null;
        }

        return ((ServerWorld) world).getPersistentStateManager().getOrCreate(AtlasManager::fromNbt, AtlasManager::new, "antiqueatlas:global_atlas_data");
    }

    @Override
    public void onInitializeClient() {

        AtlasItems.registerModItems();
